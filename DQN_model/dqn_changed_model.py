...
from Replay_buffer.dual_replay_buffer import DualReplayBuffer
...
class RepeatActionDQN(DQN):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.min_continuous_steps = kwargs.pop('min_continuous_steps', 8)   # 最小连续步数
        self.max_continuous_steps = kwargs.pop('max_continuous_steps', 20)  # 最大连续步数
    ...
    # 检查是否正在连续探索中（优先级最高）
        ...
        # 探索分支：开始新的连续探索序列
        self.current_explore_action = np.random.choice(self.useful_actions)
        self.remaining_continuous_steps = np.random.randint(self.min_continuous_steps, self.max_continuous_steps + 1) - 1
        
        return np.array([self.current_explore_action]), state